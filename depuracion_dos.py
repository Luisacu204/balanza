...
try:
    with serial.Serial(port, baud_rate, timeout=10) as ser:
        print(f"Conectado a la balanza en el puerto {port}")
        print(f"Configuración del puerto: {ser}")

        # Enviar comando inicial (ajustar si es necesario)
        # ser.write(b'\r\n')

        while True:
            raw_data = ser.readline()  # Leer datos
            print(f"Datos crudos: {raw_data}")  # Mostrar datos crudos
